...
import { CHANNEL_PRESETS, PRESET_CATEGORIES, getPresetById, NOW_PLAYING_PRESET_ID } from '../data/channelPresets.js';
...
// Priority genres: create channels first, but only include items whose lead (first) genre matches.
// Keeps categories correct (e.g. a comedy is not placed on the Drama channel).
...
export class ChannelManager {
  ...
  async generateChannelsFromPresets(
    ...
    for (let i = 0; i < presetIds.length; i++) {
      ...
      // Now Playing: a meta channel with no library items. Programs are generated by
      // ScheduleEngine from sibling channels' just-written schedule blocks.
      if (preset.id === NOW_PLAYING_PRESET_ID) {
        ...
        console.log(`[ChannelManager] Created Now Playing channel: ${channelName}`);
        i += runLength - 1;
        continue;
      }
